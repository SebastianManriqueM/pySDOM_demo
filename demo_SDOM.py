...
import sdom as sd
# from sd import run_solver, initialize_model, configure_logging
# from sd import load_data, export_results
import time
import logging

def main( with_resilience_constraints = False, case='test_data' ):
    sd.configure_logging(level=logging.DEBUG)
    n_steps = 48  # Number of steps in the simulation

    start_time = time.time()
    data = sd.load_data( "C:\\Users\\smachado\\repositories\\pySDOM\\SDOM\\Data" ) 
    elapsed_time = time.time() - start_time
    print(f"Data loading took {elapsed_time:.2f} seconds.")

    start_time = time.time()
    model = sd.initialize_model( data, n_hours = n_steps, with_resilience_constraints = with_resilience_constraints )
    elapsed_time = time.time() - start_time
    print(f"Model initialization took {elapsed_time:.2f} seconds.")

    start_time = time.time()
    best_result = sd.run_solver(model, cbc_executable_path=".\\Solver\\bin\\cbc.exe") #For now, do not change.
    elapsed_time = time.time() - start_time
    print(f"Solver run for {n_steps} steps took {elapsed_time:.2f} seconds.")

    if best_result:
        sd.export_results(model, case)
    else:
